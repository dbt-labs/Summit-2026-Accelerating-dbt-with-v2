...
Problem: This counts total line items, not unique products.
...
Why this matters: If an order contains multiple quantities of the same product, we should count that product once.

dbt v2 feature highlight: Preview CTE on item_rollup allows comparing items_count vs distinct_products_count.
...
Problem: This produces one row per product per order — not one row per order.
...
Why this matters: fct_orders must be strictly at the order grain. Any duplication here cascades into the final join.

dbt v2 feature highlight: Preview CTE on item_rollup allows confirmation of one row per order_id.
...
Problem: The joined CTE's FROM clause is from orders. Referencing order_items in the join condition is incorrect and causes either:
...
Why this matters: We want a clean 1:1 join between orders and item_rollup.

dbt v2 feature highlight:
